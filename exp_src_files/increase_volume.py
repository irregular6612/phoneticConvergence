...
def process_directory(input_dir, output_dir, volume_factor=3.0):
    ...
    # WAV 파일 목록 가져오기
    wav_files = [f for f in os.listdir(input_dir) if (f.endswith('.wav') or f.endswith('.WAV'))]
...
def main():
    ...
    input_dir_list = os.path.join(Path.home(), "Documents", "LCBL-audio-PC", "Phon-Conv")
    participant_list = os.listdir(input_dir_list)
    participant_list = [participant for participant in participant_list if "ZOOM" in participant]

    for participant in participant_list:
        input_dir = input_dir_list + "/" + participant
        output_dir = input_dir + "/50x"
        volume_factor = 50.0
