...
class AudioDeviceWindow:
    def __init__(self):
        self.window = tk.Tk()
        self.window.title('오디오 및 단어 파일 설정')
        ...
        # 기본 경로 설정
        default_practice_word_path = os.path.abspath(os.path.join(os.getcwd(), 'practice_words.xlsx'))
        default_practice_audio_path = os.path.abspath(os.path.join(os.getcwd(), 'practice_audio'))
        
        # config에서 경로 가져오기
        self.practice_word_path_var = tk.StringVar(value=config['paths'].get('practice_word_file', default_practice_word_path))
        self.practice_audio_path_var = tk.StringVar(value=config['paths'].get('practice_audio_dir', default_practice_audio_path))
        
        # 안내 메시지
        tk.Label(
            self.window,
            text='연습 단어 파일과 오디오 파일 경로를 설정해주세요.',
            font=('Arial', 12),
            wraplength=800
        ).pack(pady=20)
        ...
        # 연습 단어 파일 경로 표시 프레임
        practice_word_path_frame = tk.Frame(self.window)
        practice_word_path_frame.pack(fill='x', padx=20, pady=5)
        
        self.practice_word_path_entry = tk.Entry(
            practice_word_path_frame,
            textvariable=self.practice_word_path_var,
            width=50,
            font=('Arial', 11)
        )
        self.practice_word_path_entry.pack(side=tk.LEFT, padx=5)
        
        # 연습 단어 파일 경로 선택 버튼
        tk.Button(
            practice_word_path_frame,
            text="파일 선택",
            command=lambda: self.select_file(self.practice_word_path_var, [('Excel files', '*.xlsx')]),
            font=('Arial', 11)
        ...
        # 연습 오디오 디렉토리 경로 표시 프레임
        practice_audio_path_frame = tk.Frame(self.window)
        practice_audio_path_frame.pack(fill='x', padx=20, pady=5)
        
        self.practice_audio_path_entry = tk.Entry(
            practice_audio_path_frame,
            textvariable=self.practice_audio_path_var,
            width=50,
            font=('Arial', 11)
        )
        self.practice_audio_path_entry.pack(side=tk.LEFT, padx=5)
        
        # 연습 오디오 디렉토리 경로 선택 버튼
        tk.Button(
            practice_audio_path_frame,
            text="경로 선택",
            command=lambda: self.select_path(self.practice_audio_path_var),
            font=('Arial', 11)
        ).pack(side=tk.LEFT, padx=5)
    ...
    def check_paths(self):
        ...
        if not all([practice_word_path, practice_audio_path]):
        ...
        try:
            ...
            if '단어' not in df.columns or len(df['단어']) == 0:
                self.status_label.config(
                    text='연습 단어 파일에 "단어" 열이 없거나 단어가 없습니다.',
                    fg='red'
                )
                return
        except Exception as e:
            self.status_label.config(
                text=f'연습 단어 파일을 읽을 수 없습니다: {str(e)}',
                fg='red'
            )
            return
                
        # 연습 오디오 파일 확인
        audio_files = [f for f in os.listdir(practice_audio_path) if f.endswith('.wav')]
        if not audio_files:
            self.status_label.config(
                text='연습 오디오 디렉토리에 오디오 파일이 없습니다.',
                fg='red'
            )
            return
    ...
    def confirm(self):
        ...
        if not os.path.exists(practice_audio_path):
            try:
                ...
                messagebox.showerror("오류", f"연습 오디오 디렉토리를 생성할 수 없습니다:\n{str(e)}")
                return
    ...
    def show(self):
        ...
        self.window.destroy()
        if self.selected_device is not None and self.selected_paths is not None:
            return {
...
class MainExperimentWindow:
    def __init__(self, config):
        ...
        self.practice_word_path = config['paths']['practice_word_file']
        ...
        self.timing_data = []
        self.remaining_files = []  # remaining_files 초기화 추가
        ...
        self.practice_words = []
        self.practice_audio_files = []

    def load_practice_words(self):
        """연습용 단어 목록을 로드합니다."""
        try:
            # 파일 존재 여부 확인
            if not os.path.exists(self.practice_word_path):
                messagebox.showerror("오류", f"연습 단어 파일을 찾을 수 없습니다: {self.practice_word_path}")
                return []
            
            # 파일 읽기
            df = pd.read_excel(self.practice_word_path)
            
            # '단어' 열 존재 여부 확인
            if '단어' not in df.columns:
                messagebox.showerror("오류", "연습 단어 파일에 '단어' 열이 없습니다.")
                return []
            
            # 단어 목록이 비어있는지 확인
            if len(df['단어']) == 0:
                messagebox.showerror("오류", "연습 단어 파일에 단어가 없습니다.")
                return []
            
            return df['단어'].tolist()
            
        except Exception as e:
            messagebox.showerror("오류", f"연습 단어 파일을 읽는 중 오류가 발생했습니다: {str(e)}")
            return []
...
"""
    ...
    def initialize_stage(self, stage_number):
        ...
        if self.current_stage == 1:
            ...
            self.words = self.load_words()  # words 속성 초기화
            random.seed(42)
    ...
    def show_next_word(self):
        if hasattr(self, 'practice_mode') and self.practice_mode:
            if self.current_word_index < len(self.practice_words):
                ...
                if self.current_stage == 1:
                    self.recorder.start_recording(f"{self.participant_id}_practice_stage{self.current_stage}")
                    self.timing_data.append({
                        '단어': word,
                        '시작시간': current_time,
                        '단계': f'연습_{self.current_stage}',
                        '참가자번호': self.participant_id,
                ...
                # 연습 시행이 끝나면 실제 시행 안내 표시
                self.show_stage_instruction(self.current_stage)
                self.practice_mode = False
        else:
            # words 속성이 있는지 확인
            if not hasattr(self, 'words'):
                self.words = self.load_words()
                random.seed(42 if self.current_stage == 1 else 44)
                random.shuffle(self.words)
                self.current_word_index = 0
    ...
    def load_practice_audio_files(self):
        """연습용 오디오 파일 목록을 로드합니다."""
        self.practice_audio_files = []
        ...
        if os.path.exists(audio_dir):
            audio_files = [os.path.join(audio_dir, f) for f in os.listdir(audio_dir) 
                         if f.endswith('.wav') and not f.startswith('._')]
            self.practice_audio_files = audio_files
        else:
            print(f"연습용 오디오 디렉토리를 찾을 수 없음: {audio_dir}")
            messagebox.showerror("오류", "연습용 오디오 파일을 찾을 수 없습니다.")

    def show_practice_instruction(self, stage_number):
        """연습 시행 안내 창을 표시합니다."""
        if self.current_dialog:
            self.current_dialog.destroy()
        ...
        instruction_text = tk.Label(
            instruction_frame,
            text=f"연습 시행을 시작하겠습니다.\n\n" +
                 "이제부터 {stage_number}단계와 동일한 방식으로 진행됩니다.\n\n" +
                 "연습 시행을 통해 실험 진행 방식을 익히신 후,\n\n" +
                 "실제 실험을 시작하시면 됩니다.\n\n" +
                 "준비되셨다면 '시작하기' 버튼을 눌러주세요.",
            font=('Arial', 36),
            justify=tk.CENTER,
            wraplength=self.wrap_length
        )
        ...
        start_button = tk.Button(
            instruction_frame,
            text="시작하기",
            command=lambda: self.start_practice_after_instruction(instruction_frame, stage_number),
            font=('Arial', 24)
        )
        start_button.pack(pady=40)
    ...
    def start_practice_after_instruction(self, instruction_frame, stage_number):
        """연습 시행 안내 창을 닫고 연습 시행을 시작합니다."""
        instruction_frame.destroy()
        self.current_dialog = None
        self.initialize_practice_stage(stage_number)

    def initialize_practice_stage(self, stage_number):
        """연습 시행을 초기화합니다."""
        ...
        if stage_number == 1:
            self.main_label.config(text='연습: 단어를 소리내어 읽어주세요')
            self.instruction_label.config(text='스페이스바를 눌러 시작하세요')
            self.practice_words = self.load_practice_words()
            random.shuffle(self.practice_words)
            self.current_word_index = 0
            
        elif stage_number == 2:
            self.main_label.config(text='연습: 음성을 듣고 따라 읽어주세요')
            self.instruction_label.config(text='스페이스바를 눌러 시작하세요')
            self.load_practice_audio_files()
            if not hasattr(self, 'recorder') or not self.recorder:
                self.recorder = AudioRecorder(self.selected_device, self.folder_path)
            self.recorder.start_recording(f"{self.participant_id}_practice_stage{stage_number}")
    ...
    def start_stage(self, stage_number):
        """단계를 시작합니다. 1단계와 2단계의 경우 연습 시행을 먼저 진행합니다."""
        if stage_number in [1, 2]:
            self.show_practice_instruction(stage_number)
        else:
            self.show_stage_instruction(stage_number)
...
def create_participant_folder(participant_id, base_dir, selected_device, selected_lists, participant_info):
    """참가자 데이터 폴더를 생성합니다."""
...
class PathSettingWindow:
    def __init__(self):
        ...
        # 기본 경로 설정
        default_data_path = os.path.abspath(os.path.join(os.getcwd(), 'experiment_data'))
        default_image_path = os.path.abspath(os.path.join(os.getcwd(), 'images'))
        
        # config에서 경로 가져오기
        self.data_path_var = tk.StringVar(value=config['paths'].get('participant_data_dir', default_data_path))
        self.image_path_var = tk.StringVar(value=config['paths'].get('images_dir', default_image_path))
        
        # 이미지 경로 유효성 검사
        if os.path.exists(self.image_path_var.get()):
            # 필수 이미지 파일 확인
            ai_image_path = os.path.join(self.image_path_var.get(), 'ai.png')
            human_image_path = os.path.join(self.image_path_var.get(), 'human.png')
            
            if not (os.path.exists(ai_image_path) and os.path.exists(human_image_path)):
                missing_files = []
                ...
                messagebox.showwarning("이미지 파일 누락", 
                                     ...
                                     "올바른 이미지 경로를 선택해주세요.")
                self.image_path_var.set(default_image_path)
    ...
    def check_paths(self):
        ...
        if not all([data_path, image_path]):
            self.status_label.config(
                text='모든 경로를 입력해주세요.',
                fg='red'
            )
        ...
        # 디렉토리 경로 확인 및 생성
        for path, name in [(data_path, '실험 데이터'), (image_path, '이미지')]:
            if not os.path.exists(path):
                try:
                    os.makedirs(path)
                except Exception as e:
                    self.status_label.config(
                        text=f'{name} 경로 생성 실패: {str(e)}',
                        fg='red'
                    )
                    return
...
def main():
    while True:  # 참가자 정보 입력을 반복할 수 있도록 루프 추가
        ...
        result = device_window.show()
        
        if result is None:
            return
            
        selected_device = result['selected_device']
        practice_paths = result['selected_paths']
        
        # 연습 파일 경로를 config에 저장
        config_manager.config['paths'].update(practice_paths)
        config_manager.save_config(config_manager.config)
